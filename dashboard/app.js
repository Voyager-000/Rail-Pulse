...
function renderMap() {
  ...
  mapMarkers.forEach(m => trainMap.removeLayer(m));
  mapMarkers = [];
  
  DATA.trains.forEach(t => {
    const parts = t.route.split('→');
    let coords = STATION_COORDS[parts[0]] || STATION_COORDS[parts[1]];
    
    if (!coords) {
       const keys = Object.keys(STATION_COORDS);
       const randKey = keys[Math.floor(Math.random() * keys.length)];
       coords = [STATION_COORDS[randKey][0] + (Math.random() - 0.5) * 5, STATION_COORDS[randKey][1] + (Math.random() - 0.5) * 5];
    } else {
       coords = [coords[0] + (Math.random() - 0.5) * 1.5, coords[1] + (Math.random() - 0.5) * 1.5];
    }
...
function handleLogin() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('user-name-display').textContent = document.getElementById('login-id').value.toUpperCase();
  }, 800);
}
